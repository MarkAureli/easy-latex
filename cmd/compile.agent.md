...
5. **Detect IEEEtran and apply local overrides** — if main `.tex` file uses `\documentclass{IEEEtran}` (via `texscan.HasDocumentClass`), locally set: brace-titles=true, max-authors=5 (if unset), arxiv-as-unpublished=true
5b. **Write bibliography** — `bib.WriteBibFromCache`: extract cited keys from `.aux`/`.bcf` (`citedKeysFromArtifacts`), write only those entries to `bibliography.bib` with all config transforms (including local overrides) applied; update hash
