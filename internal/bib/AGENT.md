...
- **Crossref**: all fetched string fields run through `html.UnescapeString` to decode HTML entities (`&amp;`, `&lt;`, `&#8212;`, etc.) Crossref embeds in JSON; corrects title, author, journal/booktitle (full name), year, volume, number, pages, doi, publisher (only for `@book`/`@incollection`); title cleaned via `cleanCrossrefTitle` (converts MathML to `$...$` and Crossref face markup `<i>/<sub>/<sup>/...` to LaTeX commands); author names normalised from all-caps to title case via `normalizeAllCapsName`; **group/consortium authors** (Crossref `name` field without `given`/`family`) wrapped in braces `{Name}` so BibTeX treats them as single corporate entity; when any group author is present, all individual authors are omitted (only group authors kept); maps `type` to BibTeX entry type (`journal-article`→`@article`, `proceedings-article`→`@inproceedings`, `book-chapter`→`@incollection`, `book`/`monograph`/`edited-book`/`reference-book`→`@book`, `report`/`report-component`→`@techreport`, `dissertation`→`@phdthesis`; unknown types leave entry type unchanged); `container-title` maps to `journal` or `booktitle` based on mapped type; `@article` journal → `AbbreviateISO4` on live entry when `abbreviateJournals=true`
