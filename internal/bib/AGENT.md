...
3. Each unseen entry: `validateEntry` → Crossref/arXiv fix; store pending cache
...
- **DOI path**: dedup via `Fields["doi"]` (case-insensitive); calls `queryCrossref`; base entry type `@article`
- **arXiv path**: dedup via `Fields["eprint"]`; calls `queryArxiv`; base entry type `@misc` with `archiveprefix: {arXiv}`
...
- **Crossref**: corrects title, author, journal (full name), year, volume, number, pages, doi; `@article` journal → `AbbreviateISO4` on live entry when `abbreviateJournals=true`
- **arXiv**: corrects title, author, year
